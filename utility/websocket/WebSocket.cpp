...
std::string base64encode(unsigned char const *src, size_t n) {
	...
	EVP_EncodeInit(pCtx);
	auto *buff = new unsigned char[2 * n];
	int n_res = 0;
	int count = 0;
...
WebSocket::WebSocket() : server_fd{0}, epoll_fd(0) {}
...
int recv_data(int fd, std::string &data) {
	...
	std::string buff;
	buff.resize(max_len);
	ssize_t n = recv(fd, &buff[0], buff.size(), 0);
	if (n <= 0)
		return -1;
...
void WebSocket::start(bool &check_continue) {
	...
	while (check_continue) {
		int n = epoll_wait(epoll_fd, events.data(), events.size(), 2000);
		if (n < 0) break;
		for (int i = 0; i < n; ++i) {
			auto &ev = events[i];
